...
- 標圖版 `直標` 群組最終高度不可超過 1000px；大標1 W 上限 1400px，大標2 W 上限 1340px。
- 大標版大標1固定縮放至 W=1500px，最後一字基線位移為 -30px；此規則不可套用到標圖版。
- 大標1與大標2出現中文引號 `「」` 時，兩種符號字級設為 400pt；只有大標版的 `「` baseline shift 設為 80pt，標圖版不設定 baseline shift。若 `「` 位於行首、前方沒有文字，不設定 `「` 前方 Kerning，改為將整行大標固定向左移 140px；若 `「` 前方有文字，則將 `「` 前方與對應 `」` 後方 Kerning 都設為 -300。若去除尾端空白後最後一字是 `」`，以左側為固定點將大標寬度往右增加 70px。多個 `kerningRange` 必須依 `From` 由大到小寫入。
