...
- 大標1與大標2出現中文引號 `「」` 時，兩種符號字級設為 400pt，`「` 的 baseline shift 設為 80pt。若 `「` 位於行首、前方沒有文字，不設定 `「` 前方 Kerning，改為將整行大標固定向左移 140px；若 `「` 前方有文字，則將 `「` 前方與對應 `」` 後方 Kerning 都設為 -300。若去除尾端空白後最後一字是 `」`，以左側為固定點將大標寬度往右增加 70px。多個 `kerningRange` 必須依 `From` 由大到小寫入。
